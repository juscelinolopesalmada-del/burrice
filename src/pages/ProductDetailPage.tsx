...
const ProductDetailPage = () => {
  ...
  const isFlashSale = window.location.pathname.startsWith('/oferta-produto/') || 
                    window.location.pathname.startsWith('/brinde-produto/') || 
                    document.referrer.includes('/ofertas') || 
                    document.referrer.includes('/brinde');
  
  const rawProduct = products.find((p) => p.id === id);
  
  const product = useMemo(() => {
    if (!rawProduct) return null;
    if (!isFlashSale) return rawProduct;
    
    return {
      ...rawProduct,
      originalPrice: rawProduct.price,
      price: rawProduct.price * 0.5,
  ...
  }, [rawProduct, isFlashSale]);

  useEffect(() => {
    if (product) {
      trackViewContent({ id: product.id, name: product.name, price: product.price, zuckHash: product.zuckHash });
  ...
  const primaryVariant = useMemo(() => {
    ...
    // Find the first variant that has a price different from the base price,
    // or just the first selected variant if all have base price.
    for (let i = 0; i < product.options.length; i++) {
      const v = product.options[i].variants[selectedOpts[i] ?? 0];
      if (v && v.price !== product.price) return v;
    }
    return product.options[0]?.variants[selectedOpts[0] ?? 0];
  }, [product, selectedOpts]);
  ...
  const handleBuyNow = () => {
    ...
    addItem(variantProduct, label);
    trackAddToCart({ id: product.id, name: variantProduct.name, price: currentPrice, quantity: qty, zuckHash: product.zuckHash });
    navigate("/checkout");
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-6">
        <div className="bg-card rounded-2xl border border-border/50 p-4 md:p-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10">
            ...
            <div className="flex flex-col">
                ...
                <div className="flex gap-0.5">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star
                      key={i}
                      size={16}
                      className={i < (product.rating >= 4.9 ? 5 : Math.round(product.rating)) ? "fill-warning text-warning" : "fill-muted text-muted"}
                    />
                  ))}
                </div>
              ...
              <LowStockIndicator productId={product.id} forceStock={primaryVariant?.stock} className="mb-6" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {product.features.map((f) => (
                <div key={f} className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-semibold text-foreground uppercase tracking-wider">{f}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Brand Story Banner */}
        <div className="mt-8 bg-card rounded-2xl border border-border/50 overflow-hidden">
          <div className="grid grid-cols-1 md:grid-cols-2">
            <div className="h-64 md:h-80">
              <img 
                src="https://i.ibb.co/gMb9TjVN/loja-verdecasa.jpg" 
                alt="Verde Casa Store" 
                className="w-full h-full object-cover object-top"
              />
            </div>
            <div className="p-8 md:p-12 flex flex-col justify-center text-center md:text-left">
              <h3 className="font-heading font-bold text-2xl text-foreground mb-4 italic">Com amor, Verde Casa</h3>
              <div className="space-y-1 text-base md:text-lg text-muted-foreground font-medium">
                <p>Há 10 anos presente no mercado,</p>
                <p>levando beleza e conforto para seu lar.</p>
                <p>Com a confiança de quem possui</p>
                <p>lojas físicas em mais de 3 estados.</p>
              </div>
            </div>
          </div>
        </div>
