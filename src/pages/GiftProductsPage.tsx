...
const GiftProductsPage = () => {
  ...
  // Apply 50% discount to all products and their variants for this page
  const discountedProducts = useMemo(() => {
    return products.map(p => ({
      ...p,
      originalPrice: p.price, // Keep original price for display
      price: p.price * 0.5,
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [discountedProducts, categorySlug, query, sort, priceRange]);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <nav className="text-xs text-muted-foreground mb-6 flex gap-2">
          ...
          <span>/</span>
          <span className="text-foreground">{currentCategory?.name || "Brindes Especiais"}</span>
        </nav>
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-6">
              <p className="text-sm text-muted-foreground">{filtered.length} brindes disponíveis</p>
            ...
            {filtered.length === 0 && (
              <div className="text-center py-20 text-muted-foreground">
                <p className="text-lg">Nenhum brinde encontrado</p>
...
export default GiftProductsPage;