  ...
  return (
    <div className="group relative bg-card rounded-2xl overflow-hidden border border-border/40 hover:shadow-lg hover:border-border transition-all duration-300 flex flex-col h-full">
      ...
      <div className="relative aspect-square overflow-hidden bg-secondary/30">
        <Link to={
          window.location.pathname === "/ofertas" 
            ? `/oferta-produto/${product.id}` 
            : (window.location.pathname === "/brinde" ? `/brinde-produto/${product.id}` : `/produto/${product.id}`)
        }>
          <img
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        {/* LowStockIndicator removed from here as per user request */}

        <button
          onClick={() => {
            addItem(product);
            trackAddToCart({ id: product.id, name: product.name, price: product.price, quantity: 1, zuckHash: product.zuckHash });
          }}
